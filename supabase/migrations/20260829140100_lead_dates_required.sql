-- Dates are now mandatory on the inquiry form: the sales team ranks inquiries by
-- when the car is needed, so a lead without them cannot be triaged.
--
-- Enforced inside the function rather than as a column constraint. `create_lead`
-- is granted to anon, so the form's validation is not a boundary — but the
-- columns must stay nullable for the leads that were captured before this rule.
